...
public class SettingsActivity extends Activity {
  ...
  //        for (int k=0; k<MainActivity.dayList.get(i).get(j).size(); k++) {
  //          event=MainActivity.dayList.get(i).get(j).get(k);
  //          starredEvents+=sp.getBoolean(starPrefString+event.identifier, false) ? "1" : "0";
  //
  //        }
  //      }
  //    }
  ...
  //
  //      starredEvents+=sp.getBoolean(starPrefString+String.valueOf(MainActivity.NUM_EVENTS+i), false);
  //    }
  //
  //    String userName="";
  //
  ...
  //      Toast.makeText(this, "ERROR: Could not create output file,"+"contact "+
  //              getResources().getString(R.string.email)+" for help.", Toast.LENGTH_LONG).show();
  //      e.printStackTrace();
  //    }
  //
  //    try {
  ...
  //      os.close();
  //    } catch (IOException e) {
  //      Toast.makeText(this, "ERROR: Could not write to output file,"+"contact "+
  //              getResources().getString(R.string.email)+" for help.", Toast.LENGTH_LONG).show();
  //      e.printStackTrace();
  ...
  public void loadSettings() {
    SharedPreferences settings=
        this.getSharedPreferences(getResources().getString(R.string.sp_file_name),
  ...
  public void onPause() {
    ...
    SharedPreferences.Editor editor=
        this.getSharedPreferences(resources.getString(R.string.sp_file_name), Context.MODE_PRIVATE)
            .edit();

    editor.putBoolean(resources.getString(R.string.sp_notify_starred), notifyCB.isChecked());
    editor.putInt(resources.getString(R.string.sp_notify_time), notifyTime.getValue());

    int type;
    int id=notifyType.getCheckedRadioButtonId();
    if (id==R.id.settings_notify_vibrate) {
      type=TYPE_VIBRATE;
    } else if (id==R.id.settings_notify_ring) {
      type=TYPE_RING;
    } else if (id==R.id.settings_notify_both) {
      type=TYPE_BOTH;
    } else {
      type=-1;
    }
