...
public class SplashScreen extends CL_SplashScreen {
  ...
  public class LoadEventsTask extends AsyncTask<Integer, Integer, Integer> {
    ...
    protected Integer doInBackground(Integer... params) {
      // SETUP
      CL_MainActivity.dayStrings = getResources().getStringArray(R.array.days);
      CL_MainActivity.dayList = new ArrayList<ArrayList<ArrayList<CL_Event>>>(CL_MainActivity.dayStrings.length);

      ArrayList<ArrayList<CL_Event>> tempList;
      int i = 0;
      while (i < CL_MainActivity.dayStrings.length) {
        tempList = new ArrayList<ArrayList<CL_Event>>();
        for (int j = 0; j < 19; j++) {
          tempList.add(new ArrayList<CL_Event>());
        }
        CL_MainActivity.dayList.add(tempList);
        i++;
      }
      ...
      CL_Event event, tempEvent, prevEvent = null;
      CL_Tournament tournament;
      CL_MainActivity.allTournaments = new ArrayList<CL_Tournament>();
      ...
      for (index = 0; ; index++) {
        ...
        event = new CL_Event(identifier, tournamentID, day, hour, eventTitle, "", "",
            false, duration, false, duration, location);
        event.starred = sp.getBoolean(starPrefString + identifier, false);

        CL_MainActivity.dayList.get(day).get(hour - 6).add(0, event);
        if (event.starred)
          CL_MainActivity.addStarredEvent(event);
      }
      ...
      try {
        ...
        while ((line = reader.readLine()) != null) {
          ...
          tournamentID = -1;
          for (index = Math.max(0, CL_MainActivity.allTournaments.size() - 5); index < CL_MainActivity.allTournaments
              .size(); index++) {
            if (CL_MainActivity.allTournaments.get(index).title
          ...
          if (tournamentID > -1) {
            ...
            tournament = CL_MainActivity.allTournaments.get(tournamentID);
            if (prize > 0)
            ...
            tournamentID = CL_MainActivity.allTournaments.size();
            tournament = new CL_Tournament(tournamentID, tournamentTitle,
            ...
            CL_MainActivity.allTournaments.add(tournament);
          ...
          event = new CL_Event(identifier, tournamentID, day, hour,
          ...
          if (event.title.equalsIgnoreCase("EVENT_TITLE")) {
            ...
            if (scheduleVersion < VERSION_EVENT_CHANGED) {
              change = event.title + ": Day changed from "
                  + CL_MainActivity.dayStrings[event.day] + " to " + CL_MainActivity.dayStrings[newDay];
            }
            event.day = newDay;
          }
          ...
          if (!change.equalsIgnoreCase(""))
            CL_MainActivity.allChanges += "\t" + change + "\n\n";

          // LOAD INTO LIST
          ArrayList<CL_Event> searchList = CL_MainActivity.dayList.get(
          ...
          CL_MainActivity.dayList.get(event.day).get(event.hour - 6)
              .add(index, event);
        ...
        is.close();
        reader.close();
        CL_MainActivity.NUM_EVENTS = lineNum;
