...
public class SplashScreen extends Activity {
  ...
  public class LoadEventsTask extends AsyncTask<Integer, Integer, Integer> {
    ...
    protected Integer doInBackground(Integer... params) {
      ...
      SharedPreferences sp=getSharedPreferences(getResources().getString(R.string.sp_file_name),
      ...
      for (index=0; ; index++) {
        ...
        event=new Event(identifier, tournamentID, day, hour, eventTitle, "", "", false, duration,
      ...
      int scheduleVersion=
          sp.getInt(getResources().getString(R.string.sp_2014_schedule_version), -1);
      ...
      try {
        ...
        while ((line=reader.readLine())!=null) {
          ...
          tournamentID=-1;
          for (index=Math.max(0, MainActivity.allTournaments.size()-5);
               index<MainActivity.allTournaments.size(); index++) {
            if (MainActivity.allTournaments.get(index).title.equalsIgnoreCase(tournamentTitle)) {
              tournamentID=index;
              break;
            }
          }
          ...
          if (tournamentID>-1) {
            ...
            tournament=
                new Tournament(tournamentID, tournamentTitle, tournamentLabel, isTournamentEvent,
                    prize, gm);
          ...
          event=new Event(identifier, tournamentID, day, hour, eventTitle, eClass, format, qualify,
              duration, continuous, totalDuration, location);

          event.starred=sp.getBoolean(starPrefString+event.identifier, false);
          ...
          if (event.title.equalsIgnoreCase("EVENT_TITLE")) {
            ...
            if (scheduleVersion<VERSION_EVENT_CHANGED) {
              change=event.title+": Day changed from "+MainActivity.dayStrings[event.day]+" to "+
                  MainActivity.dayStrings[newDay];
            }
            event.day=newDay;
          }
          ...
          if (eventTitle.contains("Junior")) {
            ...
            for (; index<searchList.size(); index++) {
              ...
              if (!tempEvent.title.contains("Junior") && tempEvent.tournamentID>0) {
                break;
              }
            }
            ...
            for (; index<searchList.size(); index++) {
              tempEvent=searchList.get(index);
              if (tempEvent.eClass.length()>0 && !tempEvent.title.contains("Junior") &&
        ...
        int newVersion;
        try {
          newVersion=getPackageManager().getPackageInfo(getPackageName(), 0).versionCode;
        } catch (PackageManager.NameNotFoundException e) {
          newVersion=1000;
          Log.d(TAG,
              "ERROR: Could not get version code, defaulting to "+String.valueOf(newVersion));
          e.printStackTrace();
        }

        // save new version code
        SharedPreferences.Editor editor=sp.edit();
        editor.putInt(getResources().getString(R.string.sp_2014_schedule_version), newVersion);
        editor.apply();
